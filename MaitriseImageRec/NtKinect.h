...
# endif /* USE_THREAD */
 ...
 private:
  ...
  void initializeDepthFrame() {
	...
	ERROR_CHECK(depthFrameDescription->get_LengthInPixels(&lengthInPixels));
 ...
 public:
  ...
  void setDepth(cv::Mat& depthImage, bool raw = true) {
    ...
    for (int i = 0; i < depthImage.total(); i++) {
	  ...
	  //BYTE intensity = static_cast<BYTE>((depthBuffer[i] >= minDepth) && (depthBuffer[i] <= maxDepth) ? (depthBuffer[i] % 256) : 0);
	  //RGBQUAD pix;
	  //Vec3b color = depthImage.at<Vec3b>(i);

	  //color[0] = intensity;
	  //color[1] = intensity;
	  //color[2] = intensity;
	  //depthImage.at<UINT16>(i) = val;
	  //depthImage.at<Vec3b>(i) = color;
