...
YoloCPU::YoloCPU(float _prob, std::map<std::string,std::string> setting) {

    resizeRatio = width / (float) height; //check how it work


	width=stoi(setting["Webcam_Width"]);
	height=stoi(setting["Webcam_Height"]);

#ifdef USE_KITCHEN
    network = setting["CPUKitchen_network"];
    networkDef = setting["CPUKitchen_networkDef"];
#else
    network = setting["CPUDefault_network"];
    networkDef = setting["CPUDefault_networkDef"];
#endif
...
#ifdef USE_KITCHEN
    std::ifstream inputFile(setting["CPUKitchen_Name"]);
#else
    std::ifstream inputFile(setting["CPUDefault_Name"]);        // Input file stream object
...
inline std::vector<DetectedObject> YoloCPU::findObjects(cv::Mat color,cv::Mat depth) {
    ...
    std::vector<Mat> outs;
    std::vector<cv::String> outNames(1);
