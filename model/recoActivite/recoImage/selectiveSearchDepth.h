/**
 * \details  Selective search Depth
...
namespace selectiveDepth
{
    ...
    inline std::shared_ptr<Universe> segmentation( const cv::Mat &img, double scale, double sigma, int minSize )
    {
    ...
    inline void visualize( const cv::Mat &img, std::shared_ptr<Universe> universe )
    {
        const int height = img.rows;
        const int width = img.cols;
    ...
    struct Region
    {
        ...
        Region(
                ...
                const std::vector<float> &&depthHist,
                const std::vector<int> &&labels

        )
                : rect( rect ), size( size ), colourHist( std::move( colourHist ) ), textureHist( std::move( textureHist ) ), depthHist( std::move( depthHist ) ), labels( std::move( labels ) )
        {}
        ...
        Region& operator=( Region&& region ) noexcept
        {
            if ( this != &region )
            {
                ...
                this->depthHist = std::move( region.depthHist );
    ...
    inline std::shared_ptr<Universe> generateSegments( const cv::Mat &img, double scale, double sigma, int minSize )
    {
    ...
    inline double calcSimofDepth( const Region &r1, const Region &r2, int imSize )
    {
        assert( r1.depthHist.size() == r2.depthHist.size() );

        float sum = 0.0;

        for ( auto i1 = r1.depthHist.cbegin(), i2 = r2.depthHist.cbegin(); i1 != r1.depthHist.cend(); i1++, i2++ )
        {
            sum += std::sqrt( *i1 * *i2 );
        }
    ...
    inline std::vector<float> calcColourHist( const cv::Mat &img, std::shared_ptr<Universe> universe, int label )
    {
    ...
    inline int calcSize( const cv::Mat &img, std::shared_ptr<Universe> universe, int label )
    {
        int num = 0;
    ...
    inline cv::Mat calcTextureGradient( const cv::Mat &img )
    {
    ...
    inline std::vector<float> calcTextureHist( const cv::Mat &img, const cv::Mat &gradient, std::shared_ptr<Universe> universe, int label )
    {
    ...
    inline std::vector<float> calcDepthHist( const cv::Mat &imgDepth, std::shared_ptr<Universe> universe, int label )
    {
        std::array<std::vector<unsigned char>, 3> hsv;

        for ( auto &e : hsv )
        {
            e.reserve( imgDepth.total() );
        }
        ...
        for ( int y = 0; y < imgDepth.rows; y++ )
        {
            ...
            {
                if ( universe->find( y*imgDepth.cols + x ) != label )
                {
                    continue;
                }
                ...
                for ( int channel = 0; channel < 3; channel++ )
                {
                    hsv[channel].push_back( imgDepth.at<cv::Vec3b>( y, x )[channel] );
                }
            }
        }
    ...
    inline std::map<int, Region> extractRegions( const cv::Mat &img,const cv::Mat &depthImg, std::shared_ptr<Universe> universe )
    {
        ...
        for ( auto &labelRegion : R )
        {
            ...
            labelRegion.second.depthHist = calcDepthHist(depthImg,universe,labelRegion.first);
        }
    ...
    inline Region mergeRegions( const Region &r1, const Region &r2 )
    {
        ...
        return Region( r1.rect | r2.rect,
                       ...
                       std::move(merge( r1.depthHist, r2.depthHist, r1.size, r2.size )),
                       std::move( newLabels )
        );
    }


    inline std::vector<cv::Rect> selectiveSearchDepth( const cv::Mat &img,const cv::Mat& depth, double scale = 1.0, double sigma = 0.8, int minSize = 50, int smallest = 1000, int largest = 270000, double distorted = 5.0 )
    {
